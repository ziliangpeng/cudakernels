...
void MatmulVectorizedAuto::tune(const float *d_A, const float *d_B, float *d_C) {
    ...
    for (int i = 0; i < NUM_CANDIDATES_VEC; i++) {
        ...
        // added in PR #5 (Gemini PR #5 review caught that this calc had
        // dropped the pad). Without the pad term, candidates near the 48KB
        // ceiling could pass this static check but fail at launch.
