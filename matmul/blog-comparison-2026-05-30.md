...
| **2D blocktile (autotuned)** | [link](matmul_2d_blocktile.cu) | — | **65.1% (34.0 T)** | 68.7% (16.0 T) | **−3.6pp** ✅ |
...
**2D blocktile autotune (2026-05-30)**: 2D blocktile autotuned across 15 candidates (iterative grid expansion). Best config = `BM=BN=128, BK=16, TM=16, TN=8` (128 threads, 8×16 thread tile), **34.04 TFLOPS @ N=4096 — +52% over the hardcoded baseline (22.4 → 34.0)**. We now nearly match Simon's autotuned A100 2D blocktile (65.1% vs 68.7%) and **beat Simon's autotuned warptile when run on H100** (65.1% vs 60.9%). Three lessons: (a) BK=16 is a sweet spot, not a "deeper is better" ladder; (b) **TM and TN are NOT mirror-symmetric** — `(TM=16, TN=8)` runs 30% faster than the swapped `(TM=8, TN=16)`, because the compiler hoists `regA[i]` and longer TN causes register-allocation pressure; (c) once SM occupancy saturates, bigger blocks just add SMEM bloat without buying parallelism. See [`autotune.md`](autotune.md) and [`worklog.md`](worklog.md) Step 5 "Autotune result" for full details.
